 ...
 *
 *  Bardzo ważne:
 *  - jeśli JEST jakiekolwiek podejrzenie syntetycznych rzęs (klej, wachlarze,
 *    nienaturalna gęstość / grubość) → traktujemy jako "extensions"
 */
async function classifyLashesType(imageBase64, overrideType = null) {
  // hook na przyszłość – jeśli kiedyś przekażemy ręczne wymuszenie typu
  if (
    ...
    messages: [
      {
        ...
        content:
          ...
          "- extensions: lash extensions applied (classic, volume, hybrid, any mapping). Visible added length, density, synthetic fibres, fans, or bonding points along the lash line.\n" +
          "- lift: lash lift / lamination / lash botox style treatment. Natural lashes are chemically lifted and fixed upwards, often with tint, but WITHOUT any added synthetic extensions.\n\n" +
          "Key rules (VERY IMPORTANT):\n" +
          "- If you see ANY sign of synthetic lashes, fans, thick bases, glue bonds, very even artificial density or obviously added length → answer \"extensions\".\n" +
          "- If you are hesitating between lift and extensions → ALWAYS choose \"extensions\".\n" +
          "- Only answer \"lift\" if lashes look like NATURAL hairs that have been chemically lifted (curled upwards in a uniform arc) WITHOUT added synthetic fibres.\n" +
          "- Only answer \"natural\" if there is NO lift/lamination and NO extensions – just bare lashes (with or without mascara).\n\n" +
  ...
  // Bezpieczny parsing – sprawdzamy różne warianty odpowiedzi
  if (
    raw.includes("extension") ||
    raw.includes("synthetic") ||
    raw.includes("false lash") ||
    raw.includes("artificial")
  ) {
    return "extensions";
  }
  if (raw.includes("lift") || raw.includes("lamination")) return "lift";
  if (raw.includes("natural")) return "natural";

  // Domyślnie – dla bezpieczeństwa i biznesowo – zakładamy, że większość to stylizacje
  return "extensions";
}
