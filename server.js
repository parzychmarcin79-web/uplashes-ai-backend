...
app.post("/api/analyze-before-after", async (req, res) => {
  try {
    ...
    const completion = await client.chat.completions.create({
      model: "gpt-4.1-mini", // jeśli używasz innego modelu (np. gpt-4o), możesz tu podmienić
      temperature: 0.3,
      max_tokens: 900,
      messages: [
        {
        ...
        },
        {
          ...
          content: [
            {
              type: "text",
              text: "First image is BEFORE, second image is AFTER.",
            },
            {
              type: "image_url",
    ...
    const analysisText =
      ...
      (language === "pl"
        ? "Nie udało się wygenerować szczegółowej analizy."
        : "Could not generate a detailed analysis.");

    res.json({ analysisText });
  } catch (error) {
    console.error("Error in /api/analyze-before-after:", error);
    res.status(500).json({
      error: "Error while analyzing before/after images.",
    });
  }
});
